...
function table() {
  ...
  gettheData(db.TicketMaster, (data) => {   
    if (data) {
      console.log(data);
     
      // console.log(data.TicketId,data.Title,data.Description,data.Category,data.Priority,data.DueDate, data.Status,"Empyt");
      // var btn = document.createElement("input");
      // btn.type = "button";
      // btn.className = "btn, btn-primary";
      // btn.setAttribute(`data-TicketId`, data.TicketId);
      // btn.setAttribute(`data-status`, data.Status);
      // btn.value = actiondict[data["Status"]];
      // btn.onclick = changestatus;
      // mytable.rows.add([[data.TicketId,`<button class="btn btn-primary" onclick= editbtn(this) data-toggle="modal" data-target="#update">${data.Title}</button>`,data.Description,data.Category,"High",data.DueDate, data.Status,"btn"]]).draw();

      // mytable.rows.add([[data.TicketId,{data: data.Title,render:editbtn},data.Description,data.Category,"High",data.DueDate, data.Status,"btn"]]).draw();
      // console.log(data.Priority);
      //  console.log(actiondict[data["Status"]]);
